請產生 {{BLOCK}} E2E Regression Unit Test。

## Target

- E2E target：`{{BLOCK}}`
- Production workflow blocks：`{{WORKFLOW_BLOCKS}}`
- External mock boundaries in current evidence：`{{EXTERNAL_MOCKS}}`
...
完成前執行 task context 指定 validator command，並傳入 `--block {{BLOCK}}`。

## Planning

優先讀：
1. PROJECT_CONTRACT.md
2. `{MATERIAL_ROOT}/BlockDocs/{{BLOCK}}.md`
3. Issues/*
4. DDL/*
5. Scheduler/*
