請產生 `TokenGenerate_CommandSubmit` E2E Regression Test。

## Target

- E2E target：`TokenGenerate_CommandSubmit`
- Workflow type：`Composite`
- Production blocks：`TokenGenerate -> CommandSubmit`
- External mocks：`mq`

Composite target：只允許依序執行 TokenGenerate -> CommandSubmit；禁止拆開，也禁止加入 mapping 以外的 block。
...
-> CallMain("TokenGenerate_CommandSubmit")
...
完成前執行 task context 指定 validator，並傳入 `--block TokenGenerate_CommandSubmit`。
