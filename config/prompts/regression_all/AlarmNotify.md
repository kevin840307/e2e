請產生 `AlarmNotify` E2E Regression Test。

## Target

- E2E target：`AlarmNotify`
- Workflow type：`Action`
- Production blocks：`AlarmNotify`
- External mocks：`api`

Single target：只測 AlarmNotify，禁止自行串接其他 block。
...
完成前執行 task context 指定 validator，並傳入 `--block AlarmNotify`。
