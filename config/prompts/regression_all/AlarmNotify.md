請產生 AlarmNotify E2E Regression Unit Test。

## Target

- E2E target：`AlarmNotify`
- Production workflow blocks：`AlarmNotify`
- External mock boundaries in current evidence：`api`

這是 single-block E2E。Main 的 workflow 只包含 AlarmNotify。
...
每個 `AlarmNotify-SOP-NNN/` 是獨立 Case：
...
Workflow SQL 只描述 AlarmNotify 的 workflow definition。
...
完成前執行 task context 指定 validator command，並傳入 `--block AlarmNotify`。

## Planning

優先讀：
1. PROJECT_CONTRACT.md
2. `{MATERIAL_ROOT}/BlockDocs/AlarmNotify.md`
3. Issues/*
4. DDL/*
5. Scheduler/*
