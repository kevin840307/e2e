請產生 RouteDispatch E2E Regression Unit Test。

## Target

- E2E target：`RouteDispatch`
- Production workflow blocks：`RouteDispatch`
- External mock boundaries in current evidence：`none required by mapping`

這是 single-block E2E。Main 的 workflow 只包含 RouteDispatch。
...
每個 `RouteDispatch-SOP-NNN/` 是獨立 Case：
...
Workflow SQL 只描述 RouteDispatch 的 workflow definition。
...
完成前執行 task context 指定 validator command，並傳入 `--block RouteDispatch`。

## Planning

優先讀：
1. PROJECT_CONTRACT.md
2. `{MATERIAL_ROOT}/BlockDocs/RouteDispatch.md`
3. Issues/*
4. DDL/*
5. Scheduler/*
