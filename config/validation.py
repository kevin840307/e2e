...
class StructureChecker(Checker):
    def run(self):
        ...
        if not folder.is_dir():
            self.error("STRUCTURE_BLOCK", "E2E target folder missing", folder)
            return
        ...
        if not any(x.code.startswith("STRUCTURE_") for x in self.report.errors):
            blocks = " -> ".join(self.cfg.get("workflow_blocks", [self.block]))
            self.ok(f"structure PASS: {len(sops)} SOP(s); workflow={blocks}")
